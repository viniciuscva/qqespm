...
def compute_qq_n_matches_at_level_parallel(ilq: ILQuadTree, edge: SpatialEdge, level: int, previous_qq_n_matches: list, candidate_nodes_vi = set(), candidate_nodes_vj = set(), debug = False):
    ...
    # option 1: parallel
    #print('Before map. len(candidate_nodes_vi), len(candidate_nodes_vj), len(previous_qq_n_matches):', len(candidate_nodes_vi), len(candidate_nodes_vj), len(previous_qq_n_matches))
    ...
    #print(f"Total n-matches at level {level} for edge {str(edge)}: {len(qq_n_matches_l)}\n len(previous_qq_n_matches): {len(previous_qq_n_matches)}")
    pool_obj.close()
    return qq_n_matches_l        

# def compute_qq_n_matches_at_level_with_codes(ilq: ILQuadTree, edge: SpatialEdge, level: int, previous_qq_n_matches: list, previous_qq_n_matches_code_pairs, candidate_nodes_vi = set(), candidate_nodes_vj = set()):
#     wi, wj = edge.vi.keyword, edge.vj.keyword
#     ILQi, ILQj = ilq.quadtrees[wi], ilq.quadtrees[wj]
#     qq_n_matches_l = []
#     qq_n_matches_code_pairs_l = []
#     for i,(node_i, node_j) in enumerate(previous_qq_n_matches):
#         children_i = get_nodes_at_level(node_i, 1)
...
#         for ci in children_i:
#             for cj in children_j:
#                 if is_qq_n_match(ILQi, ILQj, ci[1], cj[1], edge):
#                     qq_n_matches_l.append((ci[1],cj[1]))
#                     qq_n_matches_code_pairs_l.append((previous_qq_n_matches_code_pairs[i][0]+ci[0],
#                                         previous_qq_n_matches_code_pairs[i][1]+cj[0]))
#     #qq_n_matches_l = list(qq_n_matches_l)
#     #print(f'Total qq_n_matches at level {level} for edge {(edge.vi.keyword, edge.vj.keyword)}: {len(qq_n_matches_l)}')
#     #print(qq_n_matches_code_pairs_l)
#     return qq_n_matches_l, qq_n_matches_code_pairs_l
                                                                       

def compute_qq_n_matches_for_all_edges(ilq: ILQuadTree, sp: SpatialPatternGraph, debug = False):
    #t0 = time()
    edges = sp.edges
    ...
    for level in range(1, max(2,depth+1)):
        ...
        for ee in exclusive_edges:
            ...
            qq_n_matches_exclusive[ee] = f_compute_qq_n_matches_at_level(ilq, ee, level, previous_qq_n_matches_exclusive[ee], candidate_nodes[ee.vi], candidate_nodes[ee.vj], debug = debug)
        ...
        for ie in inclusive_edges:
            ...
            qq_n_matches_inclusive[ie] = f_compute_qq_n_matches_at_level(ilq, ie, level, previous_qq_n_matches_inclusive[ie], candidate_nodes[ie.vi], candidate_nodes[ie.vj], debug = debug)
        ...
        # print(level, 'edges with n-matches (exc,inc):', len(qq_n_matches_exclusive), len(qq_n_matches_inclusive))
        # print(level, 'total edges:', len(edges))
        # if len(qq_n_matches_exclusive) + len(qq_n_matches_inclusive) != len(edges):
        #     print(edges)
        #     print('---')
        #     print(qq_n_matches_exclusive)
        #     print('---')
        #     print(qq_n_matches_inclusive)
        
    #print('edges with n-matches (exc,inc):', len(qq_n_matches_exclusive), len(qq_n_matches_inclusive))
    #print('Time spent on qq-n-matches:', time() - t0)
    #return {**qq_n_matches_exclusive, **qq_n_matches_inclusive}, qq_n_matches_by_level
    return {**qq_n_matches_exclusive, **qq_n_matches_inclusive}


def compute_qq_n_matches_for_all_edges_with_codes(ilq: ILQuadTree, sp: SpatialPatternGraph):
    edges = sp.edges
    vertices = sp.vertices
    keywords = [v.keyword for v in vertices]
    depth = max([get_depth(ilq.quadtrees[keyword]) for keyword in keywords])
    ...
    inclusive_edges = [edge for edge in edges if not edge.constraint['is_exclusive']]
    qq_n_matches_exclusive = dict()
    qq_n_matches_exclusive_codes = dict()
    previous_qq_n_matches_exclusive = dict()
    previous_qq_n_matches_exclusive_codes = dict()
    qq_n_matches_inclusive = dict()
    qq_n_matches_inclusive_codes = dict()
    previous_qq_n_matches_inclusive = dict()
    previous_qq_n_matches_inclusive_codes = dict()
    qq_n_matches_codes_by_level = dict()
    for ee in exclusive_edges:
        wi, wj = ee.vi.keyword, ee.vj.keyword
        previous_qq_n_matches_exclusive[ee] = [(ilq.quadtrees[wi], ilq.quadtrees[wj])]
        previous_qq_n_matches_exclusive_codes[ee] = [('', '')]
        qq_n_matches_codes_by_level[ee] = dict()
    for ie in inclusive_edges:
        wi, wj = ie.vi.keyword, ie.vj.keyword
        previous_qq_n_matches_inclusive[ie] = [(ilq.quadtrees[wi], ilq.quadtrees[wj])]
        previous_qq_n_matches_inclusive_codes[ie] = [('', '')]
        qq_n_matches_codes_by_level[ie] = dict()
    candidate_nodes = dict()
    for level in range(1, max(2,depth+1)):
        for vertex in vertices:
            candidate_nodes[vertex] = set() # it is the set of nodes that are candidates to this vertex in this level
        ...
        for ee in exclusive_edges:
            qq_n_matches_exclusive[ee], qq_n_matches_exclusive_codes[ee] = compute_qq_n_matches_at_level_with_codes(ilq, ee, level, previous_qq_n_matches_exclusive[ee], previous_qq_n_matches_exclusive_codes[ee], candidate_nodes[ee.vi], candidate_nodes[ee.vj])
            previous_qq_n_matches_exclusive[ee] = qq_n_matches_exclusive[ee]
            qq_n_matches_codes_by_level[ee][level] = qq_n_matches_exclusive_codes[ee]
            previous_qq_n_matches_exclusive_codes[ee] = qq_n_matches_exclusive_codes[ee]
            new_candidates_i, new_candidates_j = zip(*qq_n_matches_exclusive[ee])
            if candidate_nodes[ee.vi]==set(): candidate_nodes[ee.vi] = set(new_candidates_i)
            else: candidate_nodes[ee.vi] = candidate_nodes[ee.vi].intersection(set(new_candidates_i))
            if candidate_nodes[ee.vj]==set(): candidate_nodes[ee.vj] = set(new_candidates_j)
        ...
        for ie in inclusive_edges:
            qq_n_matches_inclusive[ie], qq_n_matches_inclusive_codes[ie] = compute_qq_n_matches_at_level_with_codes(ilq, ie, level, previous_qq_n_matches_inclusive[ie], previous_qq_n_matches_inclusive_codes[ie], candidate_nodes[ie.vi], candidate_nodes[ie.vj])
            previous_qq_n_matches_inclusive[ie] = qq_n_matches_inclusive[ie]
            qq_n_matches_codes_by_level[ie][level] = qq_n_matches_inclusive_codes[ie]
            previous_qq_n_matches_inclusive_codes[ie] = qq_n_matches_inclusive_codes[ie]
            new_candidates_i, new_candidates_j = zip(*qq_n_matches_inclusive[ie])
            if candidate_nodes[ie.vi]==set(): candidate_nodes[ie.vi] = set(new_candidates_i)
            else: candidate_nodes[ie.vi] = candidate_nodes[ie.vi].intersection(set(new_candidates_i))
            if candidate_nodes[ie.vj]==set(): candidate_nodes[ie.vj] = set(new_candidates_j)
    ...
    #print('edges with n-matches (exc,inc):', len(qq_n_matches_exclusive), len(qq_n_matches_inclusive))
    qq_n_matches = {**qq_n_matches_exclusive, **qq_n_matches_inclusive}
    return qq_n_matches, qq_n_matches_codes_by_level
...
def compute_qq_e_matches_for_an_edge_parallel(ilq: ILQuadTree, edge, qq_n_matches_for_the_edge, candidate_objects = dict()):
    ...
    # option 1: parallel
    #print('len(qq_n_matches_for_the_edge):', len(qq_n_matches_for_the_edge))
    #print('cumulatives during compute_qq_e_matches_for_an_edge_parallel')
    ...
    #print('Time spent on compute_qq_e_matches_for_an_edge_parallel:', time()-t0)
    #print('ended running compute_qq_e_matches_for_an_edge_parallel')
    pool_obj.close()
    return qq_e_matches

def compute_qq_e_matches_for_all_edges(ilq: ILQuadTree, sp: SpatialPatternGraph, qq_n_matches: dict, debug = True):
    #t0 = time()
    edges = sp.edges
    vertices = sp.vertices
    ...
    #print('Until end of pre-processing of qq-e-matches computation:', time() - t0)
    for edge in non_skip_edges:
        if debug:
            print('- Computing qq-e-matches for edge', edge.id)
        #print('start computing ematches for an edge')
        qq_e_matches[edge] = compute_qq_e_matches_for_an_edge_parallel(ilq, edge, qq_n_matches[edge], candidate_objects)
        #print('ended computing ematches for an edge')
        if len(qq_e_matches[edge])==0:
            return None, skip_edges, non_skip_edges
        if debug:
            print(f'- Total e-matches for edge {edge.id}: {len(qq_e_matches[edge])}')
    ...
    #print('Time spent on qq-e-matches:', time() - t0)
...
def generate_partial_solutions_ematch(qq_e_match, edge, sp):
    os, ot = qq_e_match
...
def generate_partial_solutions_from_qq_e_matches_parallel(qq_e_matches, edge, sp):
    pool_obj = ThreadPool(int(multiprocessing.cpu_count()-1))
    generate_partial_solutions_ematch_partial = partial(generate_partial_solutions_ematch, edge = edge, sp = sp)
    partial_solutions = pool_obj.map(generate_partial_solutions_ematch_partial, qq_e_matches)
...
def join_qq_e_matches2(sp: SpatialPatternGraph, qq_e_matches: dict, qq_n_matches: dict, skip_edges: list, non_skip_edges: list):
    #t0 = time()
    ...
    for edge in non_skip_edges:
        #print('filtering ematches by vertex candidates. Before:', len(qq_e_matches[edge]), type(qq_e_matches[edge]))
        #print('teste1')
        qq_e_matches[edge] = filter_qq_e_matches_by_vertex_candidates(qq_e_matches[edge], edge, candidates)
        #print('teste2')
        #print('filtering ematches by vertex candidates. After:', type(qq_e_matches[edge]))
        #print('len(emaches[edge]):', len(qq_e_matches[edge]))
        
    partial_solutions = [{vertex.id: None for vertex in sp.vertices}]
    for edge in non_skip_edges:
        partial_solutions_edge = generate_partial_solutions_from_qq_e_matches_parallel(qq_e_matches[edge], edge, sp)
    ...
    for edge in skip_edges:
        for i,solution in enumerate(partial_solutions):
            ...
            distance = os.distance(ot)
            if not(lij <= distance <= uij) or (os.relation(ot) !=  edge.constraint['relation']):
                partial_solutions[i] = None
    final_solutions = filter(lambda x: x is not None, partial_solutions)

    #print('Time spent on Joining:', time() - t0)
    return final_solutions

def join_e_matches_simple2(sp: SpatialPatternGraph, qq_e_matches: dict, qq_n_matches: dict, skip_edges: list, non_skip_edges: list):
    #t0 = time()
    non_skip_edges.sort(key = lambda e: len(qq_e_matches[e]))
    skip_edges.sort(key = lambda e: len(qq_n_matches[e]))
    ordered_edges = non_skip_edges + skip_edges
...
def QQESPM(ilq: ILQuadTree, sp, debug = True):
    if sp.__class__ == SpatialPatternGraph:
        ...
        qq_n_matches = compute_qq_n_matches_for_all_edges(ilq, sp, debug = debug)
        if qq_n_matches is None:
            return [], time() - t0, psutil.Process().memory_info().rss/(2**20)
        if debug:
            for edge in qq_n_matches:
                print(f'- Total n-matches for edge {edge.id}: {len(qq_n_matches[edge])}')
            print('Computing qq-e-matches for edges')
        # if len(sp.edges) != len(qq_n_matches):
        #     print('edges:',len(sp.edges))
        #     print('edges with nmatches:', len(qq_n_matches))
        #     for edg in sp.edges:
        #         if edg not in qq_n_matches:
        #             print(edg, 'not in nmatches')
        # avg_total_nmatches = 0
        # for edge in sp.edges:
        #     avg_total_nmatches += len(qq_n_matches[edge])
        # avg_total_nmatches /= len(sp.edges)
        # print(f'avg_total_nmatches imp.: {avg_total_nmatches}')
        
        qq_e_matches, skip_edges, non_skip_edges = compute_qq_e_matches_for_all_edges(ilq, sp, qq_n_matches, debug)
        if qq_e_matches is None:
            return [], time() - t0, psutil.Process().memory_info().rss/(2**20)
        else:
            # avg_total_ematches = 0
            # for edge in sp.edges:
            #     avg_total_ematches += len(qq_e_matches[edge])
            # avg_total_ematches /= len(sp.edges)
            # print(f'avg_total_ematches imp.: {avg_total_ematches}')
            if debug:
                print('Number of skip-edges:', len(skip_edges))
                print('Joining qq-e-matches')
            solutions = join_e_matches_simple2(sp, qq_e_matches, qq_n_matches, skip_edges, non_skip_edges)
